...
export class Backend implements backendInterface {
    ...
    async getConversationsByMostRecentMessage(): Promise<Array<ConversationView>> {
        if (this.processError) {
            try {
                const result = await this.actor.getConversationsByMostRecentMessage();
                return result;
            } catch (e) {
                this.processError(e);
                throw new Error("unreachable");
    ...
    async startConversationWith(arg0: Principal): Promise<ConversationId> {
        if (this.processError) {
            try {
                const result = await this.actor.startConversationWith(arg0);
                return result;
            } catch (e) {
                this.processError(e);
                throw new Error("unreachable");
...
function from_candid_record_n5(_uploadFile: (file: ExternalBlob) => Promise<Uint8Array>, _downloadFile: (file: Uint8Array) => Promise<ExternalBlob>, value: {
    ...
    return {
        name: value.name,
        avatar: record_opt_to_undefined(from_candid_opt_n6(_uploadFile, _downloadFile, value.avatar))
    };
}
